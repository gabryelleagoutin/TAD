...
def generate_html_output(cluster_corrected_file, output_html_file):
    ...
    with open(cluster_corrected_file, 'r') as txt_file, open(output_html_file, 'w') as html_file:
        ...
        in_cluster = False  # Variable pour vérifier si on est dans un cluster
        for line in lines:
            if line.startswith("Cluster"):
                if in_cluster:  # Si on est déjà dans un cluster, fermer la liste précédente
                    html_file.write("</ul>\n")
                html_file.write(f"<h2>{line.strip()}</h2>\n<ul>\n")
                in_cluster = True  # On est dans un nouveau cluster
            else:
        ...
        if in_cluster:  # Fermer la dernière liste ouverte
            html_file.write("</ul>\n")
