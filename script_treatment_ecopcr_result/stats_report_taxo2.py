...
def calculate_statistics(filtered_clusters, output_stats_file, output_taxo_file, output_taxo_good_discriminated_file):
    ...
    # Dictionnaires pour suivre les clusters de bonnes et mauvaises discriminations par taxonomie
    ...
    # Taxonomies qui sont à la fois bien et mal discriminées
    ...
    # Écriture des taxonomies bien discriminées dans un fichier séparé
    with open(output_taxo_good_discriminated_file, 'w') as f:
        for taxonomy in sorted(good_discriminated_taxonomies):
            f.write(f"{taxonomy}\n")

    # Écriture des taxonomies uniques dans un fichier séparé
