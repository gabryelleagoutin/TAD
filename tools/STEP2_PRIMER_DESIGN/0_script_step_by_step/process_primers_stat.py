...
def check_limiting_deg(sequence, limiting_deg):
    """
    If limiting_deg is provided, check the last 'limiting_deg' bases.
    If a degenerate base (not A,C,G,T) is found in these positions, return False.
    Otherwise, return True.

    # MODIF:
    If limiting_deg is larger than the primer length, raise an error in English.
    """
    if limiting_deg is None:
        return True

    # MODIF: Vérifier si limiting_deg > longueur du primer
    if limiting_deg > len(sequence):
        raise ValueError(
            f"Error: The limiting_deg value ({limiting_deg}) is greater than the primer length ({len(sequence)}). "
            "Please provide a smaller limiting_deg or remove the option if no filtering is required."
        )
...
def process_line(columns, og_id, og_info, nm_threshold, tm_max_threshold, tm_min_threshold, limiting_deg):
    ...
    # Ici, l'appel à check_limiting_deg va lever une ValueError si limiting_deg > taille du primer
...
Additional:
    --limiting_deg: Integer (1, 2, 3, ...). If provided, checks the last N bases for degenerate bases (not A,C,G,T). 
    If degenerate bases are found, the primer is discarded. If N is greater than the length of the primer, an error is raised.
""", formatter_class=argparse.RawTextHelpFormatter,
    ...
    parser.add_argument("--limiting_deg", type=int, help="Check last 'limiting_deg' bases for degenerate bases. If present, discard primer.")
