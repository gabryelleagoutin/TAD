...
def parse_vsearch_clusters(vsearch_cluster_file):
    ...
    with open(vsearch_cluster_file, 'r') as f:
        for line in f:
            ...
            parts = line.split('\t')
            if len(parts) == 2:
                centroid_raw = parts[0]
                centroid_raw = centroid_raw.split("|")[0].strip() 
                centroid = normalize_id(centroid_raw)

                seqs_raw = parts[1].split(',') if parts[1] else []
                sequences = []
                for s in seqs_raw:
                    s = s.split("|")[0].strip()  
                    s = normalize_id(s)    
                ...
                cluster_dict[centroid] = sequences
